...
image::Certify_LogHeaderId_In_Jenkins_CTM_Plug-in_Console.png[Logheader id In Jenkins Console,{thumbnail}]


== Changelog

=== Version 5.0.0.0 Enhance Execute Request Parameter functionality by adding Certify Result Attribute (Key/Value) pair 
    functionality

-- Create Result Attribute in Certify as per below:-
...
image::CTM_New_API.png[CTM New API for Result Attribute,{thumbnail}]
...
--  login Jenkins CTM plugin and select the same suite from the dropdown and add below Execution Parameters as per below image.
...
Result Attribute :
   --> It will be an key/value pair functionality. 
   --> The Key name should exact match with Certify Result Attribute name.
   --> Certify Process contains multiple Result Attributes.
   --> While creating different Result Attributes in certify make sure name should be different.
   --> If user used multiple Result Attribute in Certify Process then it is necessary to pass multiple Result Attributes
       from Jenkins CTM plugin as per above image else process will fail.

-- Jenkins CTM plugin will display Result Attribute details in jenkins console window as per below image
   
   image::Result_Attribute_in_Jenkins_console_window.png[Result Attribute In Jenkins Console,{thumbnail}]


-- After the execution completed, user can validate Result Attrubute value in certify as per below image
   
   image::Result_Attribute_In_Certify_Results.png[Result Attribute In Certify,{thumbnail}]
