...
Enhance Execute Request Parameter functionality by adding Certify Result Attribute (Key/Value) pair 
functionality

Create Result Attribute in Certify as per below image

image::Result_Attribute-Recordset_Jenkins.png[Certify Result Attribute,{thumbnail}]

Add Result Attribute in the process so it will be available in certify result after the execution CompletedDate 
as per below image

image::Process_With_Result_Attribute-Recordset_Jenkins.png[Result Attribute In Certify Process,{thumbnail}]

In order to handle new enhancement (Result Attribute), Continuous Testing Manager (CTM) comes up with new API as per image

image::CTM_New_API.png[CTM New API,{thumbnail}]

/api/Suite/GetAllSuitesForTenants

Create Suite in Continuous Testing Manager (CTM) by selecting same process for which result attribute was added and 
save the suite.

Login Jenkins CTM plugin and select the same suite from the dropdown and add below Execution Parameters

User can add the following execution parameters:
...
If the user wants to add multiple execution parameters for different processes, they can define them as follows:
...
NOTE: While adding multiple execution parameters make sure ProcessId will be first paraemeter 
      in every new collections as per above example.

Execution Parameter example:
...
. It will be a key/value pair functionality.
. The key name should exactly match the Certify Result Attribute name.
. Certify Process contains multiple Result Attributes.
. While creating different Result Attributes in Certify, make sure the names are different.
. If the user uses multiple Result Attributes in a Certify Process, it is necessary to pass multiple Result Attributes
  from the Jenkins CTM plugin as shown in the above image; otherwise, the process will fail.

Jenkins CTM plugin will display Result Attribute details in Jenkins console window:

image::image25.png[Result Attribute Jenkins Console,{thumbnail}]

After execution is completed, user can validate Result Attribute value in Certify:
