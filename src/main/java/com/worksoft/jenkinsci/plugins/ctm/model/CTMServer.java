  ...
  public String executeSuite(String suiteId, String suiteName, String tenantId, String tenantName,
    ...
    if (processIdCntr <= 0) {
      ...
      for (CTMProcess ctm : processMap.values())
      {
        ...
        requestBody.append("\",\"Layout\":\"").append(ctm.GetLayout()).append("\",\"Recordset\":\"").append(ctm.GetRecordset());
        requestBody.append("\",\"RecordsetMode\":\"").append(ctm.GetRecordsetMode()).append("\",\"MachineAttributes\":\"");
        requestBody.append(ctm.GetMachineAttributes().replace(':', '='));

         // If Suite having one/Multiple process and one/more result attributes and user did not pass the Result Attribute then "Attribute" should not passe to API 

         // If suite was having one process and multiple result attributes and user did not passed it then count = 0
        ...
        // If suite was having multiple process and multiple result attributes and user did not passed it then count = 0
        ...
        // If user did not pass the Result Attribute and only pass the ProcessId and select suite name from dropdown then "Attributes" should not passe to API 
